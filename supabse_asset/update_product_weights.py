...
CATEGORY_CSV_PATH = os.getenv("CATEGORY_CSV_PATH", "categories_rows (1).csv")
    ...
    for i, row in df.iterrows():
        ...
        if name.lower() in name_to_id:
            ...
            if DEFAULT_VENDOR_ID:
                insert["vendor_id"] = DEFAULT_VENDOR_ID

            res        = db.table("products").insert(insert).execute()
        ...
        db.table("nutrients").insert({
            ...
            "saturatd_fat":          sf(row.get("sat_fat")),      # ← DB typo
            ...
            "cholestrol":            sf(row.get("cholesterol")),  # ← DB typo
